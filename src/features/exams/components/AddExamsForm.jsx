    ...
    <>
      <div class="grid grid-cols-1 md:grid-cols-4 gap-x-4 gap-y-1">
        <div class="md:col-span-3">
          <Exam id={0} onValueChange={valueChangeHandler} />
        </div>
        ...
        <div class="md:col-span-4">
          <div class={`form-control w-full mt-1`}>
            ...
            <textarea
              ...
              // id={exam._id}
              onChange={(e) =>
                valueChangeHandler(0, e.target.value, "diagnostic")
              }
            ></textarea>
          </div>
        </div>
      ...
      exams_list: Object.values(examData())
        .map((obj) => {
          if (
            ...
            ["INTERNAL", "EXTERNAL"].includes(obj.type.trim()) &&
            obj.diagnostic.trim().length > 0
          )
            return {
              examName: obj.value,
              examType: obj.type,
  ...
  const onAddExam = () => {
    setExamList((prevList) => [
      ...
      <>
        <div class="grid grid-cols-1 md:grid-cols-4 gap-x-4 gap-y-1">
          <div class="md:col-span-3">
            <Exam id={prevList.length} onValueChange={valueChangeHandler} />
          </div>
          <div class="md:col-span-1">
            <SelectBoxWithFunc
              id={prevList.length}
          ...
          {(exam) => (
            <>
              ...
              <div class="grid grid-cols-1 md:grid-cols-4 gap-x-4 gap-y-1">
                ...
                <Show
                  when={exam.examType === "INTERNAL" && exam.internalResult}
                >
                  <div class="md:col-span-4">
                    <div class={`form-control w-full mt-1`}>
                      <label className="label">
                        <span className="label-text">
                          Résultats des examens internes
                        </span>
                      </label>
                      <textarea
                        className={`textarea textarea-bordered h-24`}
                ...
                <Show
                  when={exam.examType === "EXTERNAL" && exam.externalResultPDF}
                >
                  <div class="md:col-span-4">
                    <div class={`form-control w-full mt-1`}>
                      <label className="label">
                        <span className="label-text">
                          Résultats des examens externes
                        </span>
                      </label>
                      <embed
                        ...
                        width="800"
                        height="400"
                        src={`http://localhost:9000/${exam.externalResultPDF}`}
      ...
      <div class="flex flex-row-reverse">
        <div>
          <button className="btn btn-sm btn-square btn-outline">
            <BsPlusLg onClick={onAddExam} class="h-6 w-6" />
          </button>
        </div>
      </div>
