...
const fetchConstants = async () =>
  await axios.get(`http://localhost:9000/api/consultations`);

const [fetcherSignal, setFetcherSignal] = createSignal(2);

export const [consultationsRessource, { mutate, refetch }] = createResource(
  fetcherSignal(),
  fetchConstants
);
      ...
      cellRenderer: (value) => {
        ...
        return (
          <span class="font-semibold">
            {moment(cellValue).locale("fr").format("DD-MM-YYYY à hh:mm:ss")}
          </span>
        );
      },
    },
  ...
  return (
    <>
      <Show when={!consultationsRessource.loading}>
        <TitleCard title={"Liste d'attente des consultations"} topMargin="mt-1">
          <div class={"ag-theme-alpine"} style={{ height: "32rem" }}>
            <AgGridSolid
