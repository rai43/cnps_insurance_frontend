...
import {
  INITIAL_BENEFICIARY_OBJECT,
  createBeneficiaryForm,
} from "../../../schemas/beneficiarySchema";
...
const AddBeneficiaryForm = (props) => {
  ...
  const [readOnly, setReadOnly] = createSignal(
    modalState?.extraObject?.config?.openInReadOnlyMode || false
  );
  const [isLocked, setIsLocked] = createSignal(
    modalState?.extraObject?.config?.openInReadOnlyMode || false
  );

  const [loading, setLoading] = createSignal(false);
  const [commandOrderObj, setCommandOrderObj] = createSignal(
    INITIAL_BENEFICIARY_OBJECT
  );
  ...
  const submitForm = async (values) => {
    ...
    try {
      ...
      response = await axios.post(
        "http://localhost:9000/api/beneficiary",
        formData,
        {
          authorization:
            "Bearer " +
  ...
  const { form, data, reset, errors, isValid } =
    createBeneficiaryForm(submitForm);
      ...
      <form use:form>
        ...
        <div class="mb-4">
          <div class="grid grid-cols-1 md:grid-cols-3 gap-x-4 gap-y-1">
            <div>
              <InputText
                ...
                containerStyle="mt-1"
                inputStyle={
                  errors("family_name") ? "input-bordered input-error" : ""
                }
                labelTitle="Nom"
                disabled={isLocked()}
              />
            ...
            <div class="md:col-span-2">
              <InputText
                ...
                defaultValue={
                  modalState.extraObject?.data
                    ? modalState.extraObject.data.first_name
                    : commandOrderObj().first_name
                }
                containerStyle="mt-1"
                inputStyle={
                  errors("first_name") ? "input-bordered input-error" : ""
                }
                labelTitle="Prénom(s)"
                disabled={isLocked()}
              />
            ...
            <div>
              <InputText
                ...
                defaultValue={
                  modalState.extraObject?.data
                    ? modalState.extraObject.data.id_number
                    : commandOrderObj().id_number
                }
                containerStyle="mt-1"
                inputStyle={
                  errors("id_number") ? "input-bordered input-error" : ""
                }
            ...
            <div>
              <InputText
                ...
                defaultValue={
                  modalState.extraObject?.data
                    ? modalState.extraObject.data.job_title
                    : commandOrderObj().job_title
                }
                containerStyle="mt-1"
                inputStyle={
                  errors("job_title") ? "input-bordered input-error" : ""
                }
                labelTitle="Fonction"
                disabled={isLocked()}
              />
            ...
            <div>
              <InputText
                ...
                defaultValue={
                  modalState.extraObject?.data
                    ? modalState.extraObject.data.direction
                    : commandOrderObj().direction
                }
                containerStyle="mt-1"
                inputStyle={
                  errors("direction") ? "input-bordered input-error" : ""
                }
            ...
            <div>
              <InputText
                ...
                defaultValue={
                  modalState.extraObject?.data
                    ? modalState.extraObject.data.profil
                    : commandOrderObj().profil
                }
                containerStyle="mt-1"
                inputStyle={
                  errors("profil") ? "input-bordered input-error" : ""
                }
                labelTitle="Profil"
                disabled={isLocked()}
              />
            ...
            <div>
              <InputText
                ...
                defaultValue={
                  modalState.extraObject?.data
                    ? modalState.extraObject.data.index
                    : commandOrderObj().index
                }
                containerStyle="mt-1"
            ...
            <div>
              <InputText
                ...
                defaultValue={
                  modalState.extraObject?.data
                    ? modalState.extraObject.data.remark
                    : commandOrderObj().remark
                }
                containerStyle="mt-1"
                labelTitle="Remarque"
                disabled={isLocked()}
            ...
            <div>
              <SelectBox
                ...
                value={
                  modalState.extraObject?.data
                    ? modalState.extraObject.data.gender
                    : commandOrderObj().gender
                }
                name="gender"
                placeholder="Genre"
            ...
            <div>
              <InputText
                defaultValue={
                  modalState.extraObject?.data
                    ? moment(modalState.extraObject.data.dob).format(
                        "YYYY-MM-DD"
                      )
            ...
            <div>
              <InputText
                defaultValue={
                  modalState.extraObject?.data
                    ? moment(modalState.extraObject.data.doc).format(
                        "YYYY-MM-DD"
                      )
            ...
            <div>
              <InputText
                defaultValue={
                  modalState.extraObject?.data
                    ? moment(modalState.extraObject.data.doe).format(
                        "YYYY-MM-DD"
                      )
            ...
            <div class={data().agent_type === "AGENT" ? "md:col-span-3" : ""}>
              <SelectBox
                ...
                value={
                  modalState.extraObject?.data
                    ? modalState.extraObject.data.agent_type
                    : commandOrderObj().agent_type
                }
                name="agent_type"
            ...
            >
              <div class="md:col-span-2">
                <InputText
                  defaultValue={
                    modalState.extraObject?.data
                      ? modalState.extraObject.data.beneficiary_of
                      : commandOrderObj().beneficiary_of
                  }
                  type="text"
                  name="beneficiary_of"
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-x-4 gap-y-1 justify-items-center">
            <ImageUpload
              ...
              defaultValue={
                modalState.extraObject?.data
                  ? modalState.extraObject.data.picture
                  : commandOrderObj().picture
              }
        ...
        <div class="grid grid-cols-2 md:grid-cols-8 gap-x-4 gap-y-1 justify-items-center">
          ...
          <Show when={isLocked() || readOnly()}>
            ...
            >
              {isLocked() ? (
                <span class="tooltip tooltip-bottom" data-tip="Modifier">
                  <TiLockClosedOutline class="h-6 w-6" />
                </span>
              ) : (
                <span class="tooltip tooltip-bottom" data-tip="Voir Seulement">
      ...
      <Show when={Object.keys(modalState.extraObject?.data || {}).length > 0}>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-x-4 gap-y-1">
          <button
            ...
            class={
              "btn mt-2 btn-outline btn-neutral md:col-span-" +
              (loading() ? " loading" : "")
            }
          >
