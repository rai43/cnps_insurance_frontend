...
const BeneficiairiesInfo = () => {
  let qrCodeCanvasRef;
  onMount(() =>
    console.log(rightSidebarState.extraObject?.data?.beneficiaries)
  );
  ...
  return (
    <Show
      when={
        !!rightSidebarState.extraObject?.data?.beneficiaries?.partner ||
        !!rightSidebarState.extraObject?.data?.beneficiaries?.children?.length >
          0
      }
      fallback={
        <h3 class="mx-auto mt-5 font-semibold">
          Cet agent n'a pas de bénéficiaire
        </h3>
      }
    >
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-x-4 gap-y-1">
          ...
          <div class="col-start-3 col-span-2 mt-2">
            <img
              ...
              src={`http://localhost:9000/${rightSidebarState.extraObject?.data?.beneficiaries?.partner.picture}`}
              alt="Preview"
      ...
      >
        <div class="grid grid-cols-3 gap-y-1 justify-items-center">
          ...
          <h2 class="text-2xl font-semibold my-2 col-span-3">
            Enfant
            {rightSidebarState.extraObject?.data?.beneficiaries?.children
              ?.length > 0 && "s"}
          </h2>
        </div>
        <For
          each={rightSidebarState.extraObject?.data?.beneficiaries?.children}
        >
          {(child) => (
            <>
              <div class="grid grid-cols-1 md:grid-cols-3 gap-x-4 gap-y-1">
                ...
                <div class="col-start-3 col-span-2 mt-2">
                  <img
                    class="w-full h-full object-cover"
                    src={`http://localhost:9000/${child.picture}`}
                    alt="Preview"
                    style={{ "max-width": "120px", "max-height": "120px" }}
                  />
                </div>{" "}
